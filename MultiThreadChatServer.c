...
int main(int argc, char *argv[ ])
{
    ...
    while(1) {
        ...
        if(res < 0) { //MAX_CLIENT만큼 이미 클라이언트가 접속해 있다면,
            ...
            write(c_socket, greeting, strlen(greeting));//메시지출력후
            pthread_create(&thread,NULL,do_chat,(void *)&c_socket);//입장
        }
    }
}
void *do_chat(void *arg)
{
    ...
    while(1) {
        ...
        if((n = read(c_socket, chatData, sizeof(chatData))) > 0) {
			...
			for(i=0;i<MAX_CLIENT;i++){
				 if (list_c[i] != INVALID_SOCK){//초기화상태가 아니라면,
...
int pushClient(int c_socket) {
	...
	int i;
	 ...
	 for(i = 0; i < MAX_CLIENT; i++){
		...
		if(list_c[i] == INVALID_SOCK){//초기화 상태면
			list_c[i]=c_socket;// 그자리에 c소켓을넣기
